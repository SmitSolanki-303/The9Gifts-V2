...
export const EcomHeroBlock: React.FC<Props> = async ({
  ...
  const pCtaLabel = primaryCta?.label || 'Shop Gifts'
  const pCtaUrl = primaryCta?.url || '/shop'
  const sCtaLabel = secondaryCta?.label || 'Explore Hampers'
  const sCtaUrl = secondaryCta?.url || '#collection'
  
  const imageUrl = typeof image === 'object' && image?.url ? image.url : (image || 'https://images.unsplash.com/photo-1549465220-1a8b9238cd48?q=80&w=800&auto=format&fit=crop')
      ...
      <div className="container relative mx-auto overflow-hidden rounded-xl bg-gradient-to-r from-accent to-background px-6 py-16 md:px-16 md:py-24 border border-border shadow-sm flex flex-col md:flex-row items-center justify-between">
        <div className="relative z-10 flex w-full flex-col items-start justify-center text-left md:w-1/2">
          ...
          <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row">
            <Button asChild size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-full px-8">
              <Link href={pCtaUrl}>{pCtaLabel}</Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="rounded-full px-8 border-primary text-primary hover:bg-primary/10 bg-background/50 backdrop-blur-sm">
              <Link href={sCtaUrl}>{sCtaLabel}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="relative z-10 mt-12 w-full md:mt-0 md:w-5/12 flex justify-end">
          <div className="relative aspect-[4/3] w-full max-w-md overflow-hidden rounded-2xl shadow-xl bg-muted">
            {imageUrl && <img src={imageUrl as string} alt={title} className="absolute inset-0 h-full w-full object-cover" />}
            <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-transparent mix-blend-overlay"></div>
          </div>
        </div>

        <div aria-hidden className="pointer-events-none absolute right-[-5%] top-1/2 h-64 w-64 -translate-y-1/2 rounded-full bg-primary/10 blur-3xl" />
        <div aria-hidden className="pointer-events-none absolute left-[10%] top-[20%] h-40 w-40 rounded-full bg-gold/20 blur-2xl" />
      </div>
        ...
        <div className="container mt-12 overflow-x-auto pb-4">
          <div className="flex justify-start md:justify-center gap-6 min-w-max px-4">
            {categories.map((cat: any, idx: number) => {
              const catImg = typeof cat.image === 'object' && cat.image?.url ? cat.image.url : cat.image
              return (
                <Link key={cat.name || idx} href={cat.href || '#'} className="group flex flex-col items-center gap-3">
                  <div className="flex h-20 w-20 md:h-28 md:w-28 items-center justify-center rounded-full border-[3px] border-border bg-muted overflow-hidden shadow-sm transition-all duration-300 group-hover:border-primary group-hover:shadow-md group-hover:scale-105">
                    {catImg && <img src={catImg} alt={cat.name} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />}
                  </div>
